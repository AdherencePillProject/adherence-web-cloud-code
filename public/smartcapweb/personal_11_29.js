...
var day_week = true;//true for day,false for week
(function ($) {
	"use strict";
			...
			$("<div class=\"wijmo-wijev-headerbar ui-widget-header ui-corner-top\">" +
				"<span class=\"wijmo-wijev-tools\">" +
					...
					"<label for=\"" + this._uidPref + "_weekbtn\">" +
						"Week</label>" +          //edit week button here
...
"<div class=\"wijmo-wijev-loading\">" +
		...
		_onDayViewClick: function () {
			...
			var i=0;
		        var element_add="<div><span class=\"right_title\">Remainder</span>";
			for(i=0;i<10;i++){       //i equal to the number of remainders, can fetch from database
		            element_add += "<div> This is remainder" + i + "</div>" ;
		         }
		        element_add += "</div>"
		        $(".right_part").html(element_add);
                        ...
                        var i=0;
 $.ajax({
        url:'http://nusctesting2014.comuf.com/Pill_response.php', //http://nusctesting2014.comuf.com/personalResponse.php',
        type:'post',
...
//set content of right part here
			var i=0;
		  var element_add="<div><span class=\"right_title\">Progress</span>";
			for(i=0;i<j;i++){       //i equal to the number of remainders, can fetch from database
		            element_add += "<div> "+pill[i] + "<span>here goes progress<span></div>" ;
		         }
		        element_add += "</div>"
		        $(".right_part").html(element_add);
/**************add row*********************************/
var p=0;
var times=[0,10,1,1,1,2,2];
var ruler_pill_name;
var cur = $(".wijmo-wijev-daycolumn");
var column_class;
var column_date;
...
alert(column_date);
 ...
 $.ajax({
  ...
  } //this is for cur for loop
    
   var index=0;
  
    $(".wijmo-wijev-daycolumn").each(function(){
        record = record + times[index]+"</div>";
    	$(this).append(record);
        index=index+1;
    	}) ;
...
//set content of right part here
			var i=0;
		  var element_add="<div><span class=\"right_title\">Pill Symbol</span>";
			for(i=0;i<j;i++){       //i equal to the number of remainders, can fetch from database
		            element_add += "<div><span> "+pill[i] + ": </span><div class=\"dot"+i+"\"></div>" ;
		         }
		        element_add += "</div>"
		        $(".right_part").html(element_add);  
var p=0;  
...
for(s=0;s<times;s++){
    sign = "<div class=\"dot";
    sign = sign + p +"id=\""+cur_date+"_"+pill[p]+"\">"+times+"</div>";
    $(this).append(sign);
}
s=0;
...
"all-day" +
	...
	"<div class=\"wijmo-wijev-scrollpanel\">" +
		"<div class=\"wijmo-wijev-scrollcontent\">" +
			"<div class=\"wijmo-wijev-timeruler\" id=\"pill-name-ruler\">" +
...
"</div>");
			...
			if (!this._templateDayColumn) {
				...
				numbers=0;
				pill_number=1;
				while (numbers < pill_number) {
					cellClass =
						"wijmo-wijev-timeinterval weekly-pill-record ui-widget-content wijmo-wijev-pill" ;
                    cellID = "pill-record-block"	;
					/*if (isOddRow) {
						cellClass += " wijmo-wijev-oddrow";
					}*/
					//isOddRow = !isOddRow;
					timeRulerCellClass =
						"wijmo-wijev-timerulerinterval ui-widget-content wijmo-wijev-pill-" +
																			numbers;//edit the class name of each block
					s += "<div class=\"" + cellClass + "\" id=\""+numbers+"\" style=\"height: 0px\"></div>";
					/*------here is to add the left side bar , for give name of each row---*/
					
						//cellClass += " wijmo-wijev-pill-name";
						timeRulerCellClass += " wijmo-wijev-timerulerinterval-pillName";
						/*timeruler.append($("<div class=\"" + timeRulerCellClass +
							"\" style=\"height:50px\">"  + "Pill Name"+  //numbers indicate the name of each row
							"</div>"));*/
						
					/*------end of side bar, row name-------------------*/
                    numbers++;
           ...
           // $(".wijmo-wijev-daycolumn").find(".wijmo-wijev-timeinterval weekly-pill-record ui-widget-content wijmo-wijev-pill").remove();
            ...
            for (i = 0, columnCount = selectedDates.length; i < columnCount; i = i + 1) {
				...
				if (!curDayHeader) {
					...
					curDayColumn.addClass(this._dayDateToCssClass(curDayDate)); //&#38328;&#27196;&#31919;&#29999;€class&#23057;&#25795;&#21438;&#23048;&#25126;&#24366;&#37513;&#65085;&#22466;
					if (skipNextBorder) {
...
//------------------simulation edit here!!!!!!-----------------------------------------
				s +="\""+" "+"id=\""+curDate+ "\"><img id=\"mon_star\"></div>";
//------------------end for simulation edit				
				s += "</div>";
            ...
            /*---------edit month hearder here ---------------------------*/
			if (monthview.length === 0) {
                var title_label=new Date();
                title_label = new Date(selectedDate.getFullYear(),
						selectedDate.getMonth(), 3);
				monthview = $(
			"<div class=\"wijmo-wijev-view wijmo-wijev-monthview ui-widget-content\">" +
						"<h3 class=\"wijmo-wijev-header-title\" id=\""+title_label+"\">title" +
						"</h3>" +
...
"<div class=\"wijmo-wijev-event-title\">" +
appt.subject +
"</div>" +"<img src=\"images/1star.jpg\">"+//edit star here!!!7.15---------------------------------------
"</div>");
